...
# Label configuration
readonly LABEL_PREFIX="Personal,Work"
...
# Children's names
declare -a CHILDREN_NAMES=("A" "B" "C")
declare -a CHILDREN_FULL_NAMES=("A D" "B D" "C D")

# Wife's names (including nickname)
declare -a WIFE_NAMES=("Alpha" "Beta")

# School information
readonly SCHOOL_NAME="SS"
readonly SCHOOL_FULL_NAME="School"
readonly SCHOOL_DOMAIN="school.com"

# Work information
readonly WIFE_WORK="Alpha"
readonly WIFE_WORK_FULL="Alpha"
readonly WIFE_WORK_DOMAIN="alpha.int"
...
# Health Insurance
readonly HEALTH_INSURANCE="ABC"
readonly SOCIAL_SECURITY_1="CDE"
readonly SOCIAL_SECURITY_2="FGE"
...
check_sender_category() {
  ...
  # Check if sender is from school (SFS)
  ...
  # Check if sender is work-related (IVI)
...
classify_email() {
  ...
  # Prioritize school emails from SFS
  if [ "$sender_category" = "school" ]; then
    matches+=("sfs_email:critical")
    log_debug "Email from SFS detected"
  fi
...
extract_amount() {
  ...
  # Korean Won: ₩1,234 or 1,234원
  amount=$(echo "$text" | grep -oP '₩\s*[0-9,]+|[0-9,]+\s*원' | head -n 1 | tr -d '₩원 ' | tr -d ',')
...
handle_wife_mention() {
  ...
  log_info "Processing email mentioning Tharinee/Maew"
  ...
  local telegram_message="<b>💼 Wife Mentioned: Tharinee</b>
...
📧 May be work-related or personal matter"
  ...
  local digest_entry="[Wife Mentioned: Tharinee]
From: ${sender_name}
Subject: ${subject}
Action: Review email content"
...
handle_ivi_work() {
  ...
  log_info "Processing IVI work-related email"
  ...
  local telegram_message="<b>💼 IVI Work Email</b>
...
📧 Related to Tharinee's work at IVI"
  
  send_telegram "$telegram_message" "high"
  
  local digest_entry="[IVI Work Email]
From: ${sender_name}
Subject: ${subject}
Action: May require Tharinee's attention"
  
  add_to_digest "Work/IVI" "$digest_entry" "high"
...
handle_attendance() {
  ...
  log_info "Processing SFS attendance notification"
...
handle_permission_slip() {
  ...
  log_info "Processing SFS permission slip"
  ...
  local telegram_message="<b>📝 SFS Permission Slip Required</b>
...
⚠️ <b>ACTION REQUIRED:</b> Sign and return permission slip"
  ...
  local digest_entry="[SFS Permission Slip]
Child: ${child_name:-Unknown}
Event: ${event_name}
...
Action: URGENT - Sign and return"
  ...
  add_to_digest "School/SFS/PermissionSlip" "$digest_entry" "critical"
  
  if [ -n "$deadline" ]; then
    add_calendar_event "SFS Permission Slip Due: ${event_name}" "$deadline" true "" "Child: ${child_name}. Event: ${event_date}"
  fi
...
handle_parent_teacher() {
  ...
  log_info "Processing SFS parent-teacher conference"
  ...
  local telegram_message="<b>👨‍🏫 SFS Parent-Teacher Conference</b>
...
📅 Mark your calendar!"
  ...
  local digest_entry="[SFS Parent-Teacher Conference]
Child: ${child_name:-Unknown}
Teacher: ${teacher:-TBD}
...
Action: Attend conference"
  ...
  add_to_digest "School/SFS/ParentTeacher" "$digest_entry" "high"
  ...
  if [ -n "$conference_date" ]; then
    ...
    add_calendar_event "SFS Parent-Teacher: ${child_name}" "$conference_date" "$all_day" "$conference_time" "Teacher: ${teacher}"
  fi
...
handle_school_lunch() {
  ...
  log_info "Processing SFS lunch information"
  
  # Low priority - just log it
  local digest_entry="[SFS Lunch Menu]
Subject: ${subject}
Action: Review if interested"
  
  add_to_digest "School/SFS/Lunch" "$digest_entry" "low"
...
handle_discipline() {
  ...
  log_info "Processing SFS discipline notification"
  ...
  local telegram_message="<b>🚨 SFS DISCIPLINE NOTIFICATION</b>
...
⚠️ <b>URGENT:</b> Review immediately and discuss with ${child_name}"
  ...
  local digest_entry="[SFS DISCIPLINE ALERT]
Child: ${child_name:-Unknown}
Date: ${incident_date:-$(date '+%Y-%m-%d')}
Description: ${description:-See email}
Action: URGENT - Review and discuss"
  
  add_to_digest "School/SFS/Discipline" "$digest_entry" "critical"
...
handle_payment_receipt() {
  ...
  local telegram_message="<b>🧾 Payment Receipt</b>
